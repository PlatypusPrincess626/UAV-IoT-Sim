    ...
    def get_dest(self, state, full_sensor_list, model, step, p_count, targetType, targetSerial, _=None):
        ...
        if not targetType:
            ...
            if len(inactive) > 0:
                ...
                print((state[0][2] - tour_discharge) >= (0.2 * 6_800 * 1_000))
                if (state[0][2] - tour_discharge) >= (0.2 * 6_800 * 1_000):
                    ...
                    dists1= (sum(dists[0:math.ceil(len(tour)/2)]) +
                    ...
                    dists2= (sum(dists[math.ceil(len(tour)/2):]) +
                    ...
                    if (state[0][2] - tour1_discharge) >= (0.2 * 6_800 * 1_000):
                        ...
                        dists11 = (sum(distsA[0:math.ceil(len(tour1) / 2)]) +
                        ...
                        dists21 = (sum(distsA[math.ceil(len(tour1) / 2):]) +
