...
class get_ddqn_regression_agent():
    ...
    def train(self, batch_size):
        ...
        nst = np.zeros((0, self.nS))  # Next States
        for i in range(len(np_array)):  # Creating the state and next state np arrays
            st = np.append(st, np.reshape(np.array(np_array[i][0]).flatten(), (-1, 2)), axis=0)
            nst = np.append(nst, np.reshape(np.array(np_array[i][3]).flatten(), (-1, 2)), axis=0)
        st_predict = self.model.predict(st)  # Here is the speedup! I can predict on the ENTIRE batch
        nst_predict = self.model.predict(nst)
        nst_predict_target = self.model_target.predict(nst)  # Predict from the TARGET
        index = 0
        for state, action, reward, nstate, done in minibatch:
            x.append(np.reshape(np.array(state).flatten(), (-1, 2)))
            # Predict from state
