...
class get_ddaqn_agent():
    ...
    def __init__(self, nS: int, nA: int, epsilon_i: float = 1.0, epsilon_f: float = 0.0,
                 alpha: float = 0.00001, gamma: float = 0.95, epsilon: float = 0.5, mem_len: int = 2500):
        # ADF 2.0
        self.nS = nS
        self.nA = nA
        # ADF 1.0
    ...
    def build_model(self):
        ...
        x = tf.keras.layers.Dense(78, activation='relu')(inputs)
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.Dropout(0.1)(x)

        x_expanded = tf.keras.layers.Lambda(lambda y: tf.expand_dims(y, axis=1))(x)

        attn_output = tf.keras.layers.MultiHeadAttention(num_heads=self.nA, key_dim=78)(
...
class get_ddqn_agentp():
    ...
    def build_model(self):
        model = tf.keras.Sequential()  # linear stack of layers https://keras.io/models/sequential/
        model.add(tf.keras.layers.Input(shape=(self.nS,)))
        model.add(tf.keras.layers.Dense(5, activation='relu'))  # [Input] -> Layer 1
        model.add(tf.keras.layers.BatchNormalization())
        model.add(tf.keras.layers.Dropout(0.2))
        model.add(tf.keras.layers.Dense(7, activation='relu'))  # Layer 2 -> 3
        model.add(tf.keras.layers.BatchNormalization())
        model.add(tf.keras.layers.Dropout(0.2))
