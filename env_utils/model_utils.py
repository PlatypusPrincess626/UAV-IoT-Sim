...
def modify_state(state):
    ...
    # power state
    refined_state[-2] = [state[-2][1] / 1000, state[-2][2] / 6_800_000, state[-2][3] / 720]
    # expected reward
    refined_state[-1] = [state[-1][1], state[-1][2], state[-1][3]]
...
class get_ddqn_agent():
    ...
    def build_model(self):
        ...
        model.add(tf.keras.layers.Dense(54, activation='relu'))  # Layer 2 -> 3
